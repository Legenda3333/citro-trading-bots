...
-- Прогресс ЧАСТИЧНО исполненного и снятого уровня — чтобы дозаполнить его остатком.
-- {base, fee, quote} = суммарно уже исполнено по предыдущим кускам ЭТОГО уровня:
-- base — CITRO, fee — USDT, quote — USDT (для средней цены). У обычных ордеров пусто
-- (NULL). Когда уровень доисполняется, эти суммы объединяются в ОДНУ сделку статистики.
-- Для уже существующей базы добавляем столбец отдельно (идемпотентно).
alter table public.bot_orders add column if not exists partial jsonb;
